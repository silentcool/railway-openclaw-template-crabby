...
// ========== PID 1 ZOMBIE REAPER ==========
// In containers, Node.js often runs as PID 1 but doesn't reap adopted orphan
// processes. When grandchild processes (clamscan, curl, bash scripts from cron
// jobs) exit after their parent dies, they get reparented to PID 1 and become
// zombies. The proper fix is running under tini (see package.json "start"),
// but as a fallback we install a SIGCHLD handler that spawns a tiny subprocess
// to trigger Node's internal waitpid loop and reap zombies.
if (process.pid === 1) {
  const reapZombies = () => {
    try {
      // spawnSync triggers Node's internal waitpid(-1, WNOHANG) loop for all
      // children including adopted orphans, which reaps any pending zombies.
      childProcess.spawnSync("/bin/true", [], { timeout: 1000, stdio: "ignore" });
    } catch {
      // ignore — best-effort
    }
  };
  process.on("SIGCHLD", reapZombies);
  // Also reap periodically in case SIGCHLD was missed
  setInterval(reapZombies, 30_000);
  console.log("[init] PID 1 zombie reaper installed (prefer running under tini)");
}
